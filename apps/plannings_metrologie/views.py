from django.views.generic import TemplateView
from apps.core.mixins import ModulePermissionMixin

class PlaceholderDashboardView(ModulePermissionMixin, TemplateView):
    template_name = 'placeholder_dashboard.html'
    action_name = 'view'
    
    @property
    def module_name(self):
        tab = self.request.GET.get('tab')
        if tab in ['planning_etalonnage', 'planning_maintenance']:
            return tab
        return 'planning_etalonnage' # default fallback
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        tab = self.request.GET.get('tab')
        if tab == 'planning_maintenance':
            context['title'] = "Planning Maintenance"
        else:
            context['title'] = "Planning Étalonnage"
        return context

